        ...
        for epoch in range(CFG['EPOCHS']):
            ...
            for images, labels in progress_bar:
                ...
                with torch.no_grad():
                    ...
                    # 1. 모든 teacher의 output을 1번만 계산하고 저장
                    for teacher in teacher_models:
                        ...
                        outputs = teacher['model'](teacher_images)
                        teacher_outputs_list.append(outputs)

                        # CE loss 계산 (teacher output vs ground truth)
                        loss = criterion(outputs, labels)
                        ce_losses.append(loss.item())

                    # 2. 각 teacher의 CE loss에 반비례하는 weight 계산
                    loss_array = torch.tensor(ce_losses)
                    # 방법 1: 반비례 방식
                    eps = 1e-8
                    inv_losses = 1. / (loss_array + eps)
